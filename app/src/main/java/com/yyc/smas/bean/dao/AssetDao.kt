...
interface AssetDao {
     ...
     *  查询全部，不包括异常
     */
    @Query("SELECT * FROM AssetBean WHERE asset_roNo =:roNo AND asset_orderNo = :orderId AND asset_inventoryStatus != 3 AND asset_companyId = :companyId")
     ...
     *  查询异常订单
     */
    @Query("SELECT * FROM AssetBean WHERE asset_orderNo = :orderId AND asset_inventoryStatus = 3 ")
    fun findFildId( orderId: String?): List<AssetBean>
    @Query("SELECT * FROM AssetBean WHERE asset_orderNo = :orderId AND asset_labelTag =:LabelTag AND (asset_inventoryStatus = 1 OR asset_inventoryStatus = 3) AND asset_roNo = :userId AND asset_companyId = :companyId")
    fun findFildId(orderId: String?,LabelTag: String?, userId: String?, companyId: String?): List<AssetBean>
     ...
     *  根据用户ID和订单查询全部
     */
    @Query("SELECT * FROM AssetBean WHERE asset_orderNo = :orderId AND asset_roNo = :roNoId AND asset_companyId = :companyId AND (asset_inventoryStatus = 1 OR asset_inventoryStatus = 3)")
    fun findById(orderId: String?, roNoId: String?, companyId: String?): List<AssetBean>

    @Query("SELECT * FROM AssetBean WHERE asset_orderNo = :orderId AND asset_roNo = :roNoId AND asset_companyId = :companyId AND asset_inventoryStatus = 3")
